...
$SHOULD_BUILD_AIRFLOW = [regex]::Match($yml_content, 'build_airflow: "(.+?)"').Groups[1].value
...
$AIRFLOW_VERSION = [regex]::Match($yml_content, 'airflow: "(.+?)"').Groups[1].value
...
function cleanContainers() {
  ...
  if($SHOULD_BUILD_AIRFLOW){
    docker ps -a | Select-String -Pattern 'airflow' | ConvertFrom-String | %{docker stop $_.P1; docker rm $_.P1}
  }
}
...
function cleanImages() {
  ...
  if($SHOULD_BUILD_AIRFLOW){
    docker images | Select-String -Pattern 'airflow' | ConvertFrom-String | %{docker rmi -f $_.P3}
  }
